...
class PyMata:
   ...
   # each byte represents a digital port and its value contains the current port settings
	...
	def servo_config_ex(self, pin, angle, msec, min_pulse=450, max_pulse=2850):
        """
        This method is added dor 86Duino Servo library by Acen.
        """
        # self.set_pin_mode(pin, self.SERVO, self.OUTPUT)
        data = [pin, angle & 0x7f, angle >> 7, msec & 0x7F, msec >> 7, min_pulse & 0x7f, min_pulse >> 7, max_pulse & 0x7f, max_pulse >> 7]
